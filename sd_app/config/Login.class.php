...
class Login {
    ...
    function  __construct($tabela = 'usuarios', $campoID = 'id', $campoLogin = 'email', $campoSenha = 'senha') {
            // Iniciando sessão
            session_start();
            // Definindo atributos
            $this->tabela = $tabela;
    ...
    function logar($login, $senha, $redireciona = null) {
        ...
        if(mysql_num_rows($query) > 0)
        {
            ...
            $_SESSION['img'] = $usuario->{'urlFotoPerfil'};
    ...
    function verificarLogin($redireciona = null) {
        // Se as sessões estiverem setadas
        if(isset($_SESSION[$this->campoID]) and isset($_SESSION[$this->campoLogin]) and isset($_SESSION[$this->campoSenha])){
            if ($redireciona !== null){
                header("Location: {$redireciona}");
